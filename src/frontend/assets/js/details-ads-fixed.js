...
function renderAdHTML(ad) {
  ...
  const imageGallery = ad.images && ad.images.length
    ? ad.images.map((img) => {
        let imgSrc = img.url_img;
        if (imgSrc) {
          imgSrc = imgSrc.replace(/\/uploads\/+/g, '/uploads/');
          if (!imgSrc.startsWith('/uploads/')) {
            imgSrc = `/uploads/${imgSrc.replace(/^\/+/, '')}`;
          }
        }
        console.log('URL de imagen corregida:', imgSrc);
        return `<img src="${imgSrc}" alt="${ad.title}" class="img-thumbnail">`;
      }).join("")
...
async function renderRelatedAds(categoryId, currentAdId) {
  ...
  try {
    ...
    ads.forEach((ad, index) => {
      ...
      let img = ad.images?.[0]?.url_img?.replace(/\/uploads\/+/g, '/uploads/') || "/assets/images/default.png";
